...
class PasswordResetService {
    ...
    async requestPasswordReset(email) {
        ...
        try {
            ...
            logger_1.default.info('Password reset email sent', { userId: user.id, email });
        }
        catch (error) {
            logger_1.default.error('Failed to send password reset email', { userId: user.id, email, error });
            throw new errors_1.ValidationError('Failed to send password reset email');
        }
        return { message: 'If the email exists, a password reset link has been sent' };
    }
    /**
    ...
    async resetPassword(token, newPassword) {
        ...
        if (resetToken.expiresAt < new Date()) {
            ...
            throw new errors_1.ValidationError('Reset token has expired');
        }
        // Check if token has been used
        if (resetToken.used) {
            throw new errors_1.ValidationError('Reset token has already been used');
        }
