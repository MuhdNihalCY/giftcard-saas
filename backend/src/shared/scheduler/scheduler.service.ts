...
import { giftCardExpiryQueue, expiryRemindersQueue, cleanupTokensQueue } from '../../infrastructure/queue';
...
export class SchedulerService {
    ...
    cron.schedule('0 2 * * *', async () => {
      ...
      try {
        ...
        for (const giftCard of giftCards) {
          await giftCardExpiryQueue.add('check-expiry', { giftCardId: giftCard.id });
        }
  ...
  scheduleIPTrackingCleanup() {
    cron.schedule('0 4 * * 0', async () => {
      ...
      try {
        ...
        const deleted = await ipTrackingService.cleanOldRecords(90);
  ...
  schedulePayoutProcessing() {
    cron.schedule('0 * * * *', async () => {
      ...
      try {
        const payoutService = (await import('../../services/payout.service')).default;
    ...
    cron.schedule('0 8 * * *', async () => {
      ...
      try {
        ...
        const settings = await prisma.payoutSettings.findMany({
          where: { payoutSchedule: 'DAILY', isActive: true },
          include: {
            merchant: { select: { id: true, merchantBalance: true } },
          },
        });

        for (const setting of settings) {
          try {
            const availableBalance = await payoutService.calculateAvailableBalance(setting.merchantId);
            ...
            if (availableBalance >= minimumAmount) {
              ...
              logger.info('Daily payout requested', { merchantId: setting.merchantId, amount: availableBalance });
            }
          } catch (error: any) {
            logger.error('Failed to process daily payout', { merchantId: setting.merchantId, error: error.message });
          }
        }
  ...
  scheduleWeeklyPayouts() {
    cron.schedule('0 8 * * 1', async () => {
      ...
      try {
        ...
        const settings = await prisma.payoutSettings.findMany({
          where: { payoutSchedule: 'WEEKLY', isActive: true },
          include: {
            merchant: { select: { id: true, merchantBalance: true } },
          },
        });
        ...
        for (const setting of settings) {
          try {
            ...
            if (availableBalance >= minimumAmount) {
              ...
              logger.info('Weekly payout requested', { merchantId: setting.merchantId, amount: availableBalance });
            }
          } catch (error: any) {
            logger.error('Failed to process weekly payout', { merchantId: setting.merchantId, error: error.message });
          }
        }
  ...
  scheduleMonthlyPayouts() {
    cron.schedule('0 8 1 * *', async () => {
      ...
      try {
        ...
        const settings = await prisma.payoutSettings.findMany({
          where: { payoutSchedule: 'MONTHLY', isActive: true },
          include: {
            merchant: { select: { id: true, merchantBalance: true } },
          },
        });
        ...
        for (const setting of settings) {
          try {
            ...
            if (availableBalance >= minimumAmount) {
              ...
              logger.info('Monthly payout requested', { merchantId: setting.merchantId, amount: availableBalance });
            }
          } catch (error: any) {
            logger.error('Failed to process monthly payout', { merchantId: setting.merchantId, error: error.message });
          }
        }
      ...
      try {
        ...
        for (const payout of failedPayouts) {
          try {
            ...
            logger.info('Failed payout retried', { payoutId: payout.id, retryCount: payout.retryCount + 1 });
          } catch (error: any) {
            logger.error('Failed to retry payout', { payoutId: payout.id, error: error.message });
          }
        }
