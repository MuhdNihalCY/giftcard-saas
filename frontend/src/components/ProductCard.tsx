...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Card className="hover:shadow-gold-glow-sm transition-all duration-300 hover:scale-105 h-full flex flex-col">
      {product.image && (
      ...
      <CardHeader>
        ...
        {product.merchant.businessName && (
          <p className="text-sm text-plum-300 mt-1">{product.merchant.businessName}</p>
        )}
      </CardHeader>
      <CardContent className="flex-1 flex flex-col">
        ...
        <div className="mb-6">
          <div className="space-y-1">
            <p className="text-2xl font-serif font-bold bg-gold-gradient bg-clip-text text-transparent">
              {displaySalePrice}
            </p>
            {displaySalePrice !== displayGiftCardValue && (
            ...
            {displaySalePrice === displayGiftCardValue && (
              <p className="text-sm text-plum-300">Gift Card Value</p>
            )}
          </div>
          {product.allowCustomAmount && (
            <p className="text-sm text-plum-300 mt-2">Custom amounts available</p>
          )}
        </div>
